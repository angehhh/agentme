...
function OpusProgressOverlay({
  ...
  const phase =
    variant === 'analysis'
      ? pct < 22 ? 'Leyendo subtítulos del vídeoâ€¦' :
        pct < 42 ? 'Analizando ritmo y picos de retenciónâ€¦' :
...
/** Copy, hashtags, checklist y metadatos del clip (modal <<INFORMACIÃ“N>> Pro). */
...
export default function SocialView({
  ...
  /** Ãndice del clip que se está renderizando a MP4, o null */
  ...
  /** URLs firmadas en Supabase (MP4 ya generados) por índice de clip â€” vista previa sin re-render */
  ...
  /** Modal <<INFORMACIÃ“N>> (contenido premium por clip) */
  const [ytClipInfoModalIndex, setYtClipInfoModalIndex] = useState<number | null>(null)
  /** Modal <<INFORMACIÃ“N>> en la rejilla del proyecto en la nube (Pro: guía completa si el plan lo trae) */
  ...
  /**
   * Origen del pack de clips en pantalla: `analysis` = último â€œAnalizarâ€ (persistido así en sessionStorage);
  ...
  /** Si no hay proyectos en la nube ni sesión abierta, no mostrar un borrador viejo de sessionStorage (evita â€œ0 proyectosâ€ + clips antiguos). */
  useEffect(() => {
  ...
  function clipsToText(
    ...
    clips.forEach((c, i) => {
      lines.push(
        `## Clip ${i + 1} · ${formatMmSs(c.start_sec)} â€“ ${formatMmSs(c.end_sec)}`,
        `**${c.title}**`,
        ...
        `Estilo captions dinámicos: ${c.dynamic_caption_style || 'â€”'}`,
        '',
        `Checklist edición:`,
    ...
    <div style={{ padding: 32, maxWidth: 860 }}>
            ...
            >
              ...
              <div>
                ...
                <span style={{
                  ...
                  URL â†’ IA â†’ proyecto · estilo Opus
                </span>
                <p style={{ fontSize: 13, color: T.ink60, lineHeight: 1.55, margin: 0 }}>
        ...
        {settingsLoading ? (
          ...
          <>
            ...
            <input
              ...
              placeholder="Ej. fitness para gente ocupada, SaaS B2B, recetas veganas rápidasâ€¦"
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
              ...
              >
                {genLoading
                  ? `Generando ${calDays} día${calDays > 1 ? 's' : ''}â€¦`
                  : isPro
                    ? 'Generar semana completa â€” 7 días (IA)'
                    : 'Generar vista previa â€” 3 días (IA)'}
              </button>
            </div>
      ...
      <div style={{ background: T.white, borderRadius: 16, padding: '24px 28px', border: `1px solid ${T.ink10}`, marginBottom: 24 }}>
        ...
        <input
          ...
          placeholder="Ej. Por qué tu rutina de skincare no funciona, errores al invertir si eres principianteâ€¦"
          style={{ ...inputStyle, marginBottom: 14 }}
        />
      ...
      <div style={{
        ...
        {ytOpenSessionId ? (
          <div style={{
            ...
            {ytSessionDetailLoading ? (
              <p style={{ color: '#888', fontSize: 13 }}>Cargando informaciónâ€¦</p>
            ) : null}
      ...
      <div style={{ background: T.white, borderRadius: 16, padding: '24px 28px', border: `1px solid ${T.ink10}`, marginBottom: 24 }}>
        ...
        <input
          ...
          placeholder="YouTube: https://youtube.com/watch?v=â€¦ · youtu.be/â€¦"
          style={{ ...inputStyle, marginBottom: 14 }}
        />
        <label style={{ fontSize: 11, fontWeight: 700, color: T.ink40, display: 'block', marginBottom: 6 }}>
        ...
        >
          {ytLoading ? 'Analizando vídeo con IAâ€¦' : 'Analizar y generar shorts'}
        </button>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 12, fontWeight: 700, color: T.coral, margin: 0 }}>
                      {formatMmSs(c.start_sec)} â€“ {formatMmSs(c.end_sec)} · ~{Math.round(c.end_sec - c.start_sec)} s
                    </p>
                  </div>
                  <a
            ...
            <div style={{
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <p style={{ margin: '6px 0 0', fontSize: 12, color: T.ink40 }}>
                  Clip {ytClipInfoModalIndex + 1} · {formatMmSs(ytClips[ytClipInfoModalIndex].start_sec)} â€“{' '}
              ...
              {!isPro ? (
                <div style={{
                  ...
                  <p style={{ margin: 0, fontSize: 13, color: '#d1d5db', lineHeight: 1.55 }}>
                    ...
                    publicar, hashtags, plataformas, checklist de edición, textos en pantallaâ€¦).
                  </p>
                </div>
              ) : null}
              {isPro && ytCloudClipInfoModal.fallbackDesc ? (
