...
function missionModeLabel(mode: string | undefined) {
    if (mode === 'social')
        return 'Social · Calendario editorial';
    if (mode === 'social_hook')
...
function MissionCard({ mission, userId, supabase }: {
    ...
    return (<div style={{ background: T.white, borderRadius: 14,
      ...
      <div onClick={toggle} style={{ padding: '18px 22px', cursor: 'pointer',
            ...
            transition: 'background .12s' }} onMouseEnter={e => (e.currentTarget.style.background = T.paper)} onMouseLeave={e => (e.currentTarget.style.background = T.white)}>
        ...
        <div style={{ flexGrow: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 4 }}>
            <p title={mission.goal || undefined} style={{
      ...
      {open && isSocialMission(mission.mode) && (<div style={{ padding: '16px 22px 20px', borderTop: `1px solid ${T.ink10}` }}>
      ...
      {open && !isSocialMission(mission.mode) && (<div style={{ padding: '16px 22px 20px' }}>
          {!loading && jobs.length > 0 && (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <span style={{ fontSize: 12, fontWeight: 600, color: T.ink40 }}>
                {jobs.length} ofertas guardadas
              </span>
              <button onClick={handleShare} disabled={sharing} style={{ display: 'flex', alignItems: 'center', gap: 6,
          ...
          {loading ? (<div style={{ display: 'flex', alignItems: 'center', gap: 10,
              ...
              Cargando ofertas...
            </div>) : jobs.length === 0 ? (<p style={{ fontSize: 13, color: T.ink40, padding: '8px 0' }}>
              No hay ofertas guardadas para esta búsqueda.
            </p>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {jobs.map((job, i) => (<div key={job.id} style={{ display: 'flex', alignItems: 'center', gap: 14,
                  ...
                  <div style={{ flexGrow: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center',
                      ...
                      {job.company && (<span style={{ display: 'flex', alignItems: 'center',
                            gap: 4, fontSize: 12, color: T.ink60 }}>
                          {IC.building} {job.company}
                        </span>)}
                      {job.location && (<span style={{ display: 'flex', alignItems: 'center',
                            gap: 4, fontSize: 12, color: T.ink60 }}>
                          {IC.mapPin} {job.location}
                        </span>)}
                      {job.posted && (<span style={{ display: 'flex', alignItems: 'center',
                            gap: 4, fontSize: 11, color: T.ink40 }}>
                          {IC.clock} {job.posted}
                        </span>)}
                    </div>
                  </div>
                  {job.url && (<a href={job.url} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 5,
                            fontSize: 12, fontWeight: 600, color: T.ink,
                            textDecoration: 'none', flexShrink: 0,
                            padding: '7px 12px', borderRadius: 7,
                            border: `1.5px solid ${T.ink10}`, background: T.white,
                            transition: 'all .12s' }} onMouseEnter={e => {
...
export default function MissionsView({ userId, supabase, onNewMission, }: {
    userId: string;
    supabase: SupabaseClient;
    onNewMission: () => void;
}) {
    ...
    return (<div style={{ padding: 32, maxWidth: 860 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start',
        ...
        <button onClick={onNewMission} style={{ display: 'flex', alignItems: 'center', gap: 7,
            ...
            whiteSpace: 'nowrap', flexShrink: 0, transition: 'opacity .15s' }} onMouseEnter={e => (e.currentTarget.style.opacity = '.85')} onMouseLeave={e => (e.currentTarget.style.opacity = '1')}>
          {IC.plus} Nueva misión
        </button>
      </div>

      
      {missions.length > 0 && (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)',
      ...
      {missions.length === 0 ? (<div style={{ background: T.white, borderRadius: 16, padding: '56px 40px',
          ...
          <button onClick={onNewMission} style={{ display: 'inline-flex', alignItems: 'center', gap: 7,
          ...
          {missions.map(m => (<MissionCard key={m.id} mission={m} userId={userId} supabase={supabase}/>))}
        </div>)}
    </div>);
}
