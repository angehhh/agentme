...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   LOCATION AUTOCOMPLETE â€” Nominatim
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  ...
  const fetchSuggestions = useCallback(async (q: string) => {
    ...
    try {
      ...
      for (const place of data) {
        ...
        if (!seen.has(key) && city && (
          ['city','town','village','municipality'].includes(place.type) ||
      ...
      if (items.length === 0) {
        ...
        setSuggestions(fb); setOpen(fb.length > 0)
      } else {
    ...
    <div ref={wrapRef} style={{ position: 'relative' }}>
      <div style={{ position: 'relative' }}>
        ...
        <input
          ...
          placeholder="Madrid, Barcelona, London..."
          autoComplete="off"
          style={{
      ...
      {open && suggestions.length > 0 && (
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: T.white,
          ...
          {suggestions.map((s, i) => (
            <div key={i} onMouseDown={e => { e.preventDefault(); select(s) }} onMouseEnter={() => setHighlighted(i)}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,.55)',
      ...
      <div style={{ background: T.white, borderRadius: 20, padding: 40, width: '100%',
        maxWidth: 520, boxShadow: '0 32px 80px rgba(0,0,0,.22)', animation: 'slideUp .3s ease' }}
        ...
        {!saved ? (
          <>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              <button onClick={() => { if (goal.trim()) setSaved(true) }} disabled={!goal.trim()}
                style={{ flex: 1, padding: '13px 0', borderRadius: 9,
              ...
              <button onClick={onClose}
                style={{ padding: '13px 20px', borderRadius: 9, background: 'transparent',
          ...
          <div style={{ textAlign: 'center', padding: '16px 0' }}>
            <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'rgba(40,200,64,.1)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              margin: '0 auto 20px', color: T.green, fontSize: 24 }}>âœ“</div>
...
const JOB_CATEGORIES = [
  { label: 'Marketing digital',      icon: 'ðŸ“¢' },
  { label: 'Marketing y ventas',     icon: 'ðŸ“ˆ' },
  { label: 'SEO / SEM',              icon: 'ðŸ”' },
  ...
  { label: 'Desarrollador Fullstack',icon: 'ðŸ”§' },
  { label: 'React Developer',        icon: 'âš›ï¸' },
  { label: 'Python Developer',       icon: 'ðŸ' },
  { label: 'DevOps / Cloud',         icon: 'â˜ï¸' },
  { label: 'Data Analyst',           icon: 'ðŸ“Š' },
  { label: 'Data Scientist',         icon: 'ðŸ§¬' },
  { label: 'Machine Learning',       icon: 'ðŸ¤–' },
  { label: 'Product Manager',        icon: 'ðŸŽ¯' },
  { label: 'Project Manager',        icon: 'ðŸ“‹' },
  { label: 'Scrum Master',           icon: 'ðŸ”„' },
  { label: 'Recursos Humanos',       icon: 'ðŸ¤' },
  { label: 'Customer Success',       icon: 'ðŸŒŸ' },
  { label: 'Ventas / Sales',         icon: 'ðŸ’¼' },
...
function QueryInput({ value, onChange, onEnter }: {
  ...
  const handleKey = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (open && filtered.length > 0) {
      ...
      if (e.key === 'ArrowUp')    { e.preventDefault(); setHighlighted(h => Math.max(h - 1, 0)); return }
      if (e.key === 'Enter' && highlighted >= 0) { e.preventDefault(); select(filtered[highlighted]); return }
      if (e.key === 'Escape')     { setOpen(false); return }
    }
    if (e.key === 'Enter') onEnter()
  }
    ...
    <div ref={wrapRef} style={{ position: 'relative' }}>
      ...
      {open && filtered.length > 0 && (
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: T.white,
          ...
          {filtered.map((cat, i) => (
            <div key={cat.label} onMouseDown={e => { e.preventDefault(); select(cat) }} onMouseEnter={() => setHighlighted(i)}
...
function OpportunityView({ profile, onProfileUpdate }: {
  ...
  const [marketStats,    setMarketStats]    = useState<{ totalLinkedIn: number; cityDistribution: Record<string, number> } | null>(null)
  const [marketInsights, setMarketInsights] = useState<{ trends: string; salaryRange: string; topCities: { city: string; demand: string }[]; tip: string } | null>(null)
  ...
  useEffect(() => {
    const loadFavorites = async () => {
      ...
      if (data) setFavorites(new Set(data.map(f => f.url)))
    }
    loadFavorites()
  }, [profile.id])

  const handleShare = async () => {
    if (sharing || !jobs.length) return
    setSharing(true)
    try {
  ...
  const runAgent = async () => {
    ...
    setLoading(true); setError(''); setLimitError(''); setJobs([]); setRan(false)
    setExpandedSearch(false); setSearchedIn(''); setAiSummary('')
    setMarketStats(null); setMarketInsights(null); setShareUrl(''); setSharing(false)
    try {
      ...
      if (data.error === 'limit_reached') {
        setLimitError(data.message); return
      }
      if (!res.ok || data.error) {
        setError(data.error || 'Error al ejecutar el agente'); return
      }

      const incoming = (data.jobs || []) as JobResult[]
      const withTips = incoming.map((job: JobResult) => {
        if (job.tip) return job
      ...
      if (data.actionsRemaining !== undefined) setActionsRemaining(data.actionsRemaining)
    ...
    <div style={{ padding: 32, maxWidth: 860 }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
          ...
          {isFree && (
            <div style={{ fontSize: 12, fontWeight: 600, color: T.ink40,
      ...
      <div style={{ background: T.white, borderRadius: 16, padding: 24,
        ...
        <div style={{ marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            {experience && (
              <button onClick={() => setExperience(null)}
                style={{ fontSize: 10, color: T.ink40, background: 'none', border: 'none',
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {EXP_LEVELS.map(exp => (
              <button key={exp.id} onClick={() => toggleFilter(experience, exp.id, setExperience)}
                style={chipStyle(experience === exp.id)}>
                {exp.label}
              </button>
            ))}
          </div>
        ...
        {loading && (
          <div style={{ marginTop: 20, padding: '16px 18px', background: T.paper,
      ...
      {/* Expanded search warning */}
      {ran && expandedSearch && location && (
      ...
      {/* Market stats â€” basic line (always) */}
      {ran && marketStats && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16,
          ...
          <div>
            ...
            {Object.keys(marketStats.cityDistribution).length > 1 && (
              <p style={{ fontSize: 12, color: T.ink40, marginTop: 3 }}>
      ...
      {/* Pro: AI Summary + Market Insights unified */}
      {ran && !isFree && (aiSummary || marketInsights) && (
        <div style={{ background: 'linear-gradient(135deg, #0C0C0C 0%, #1a1a2e 100%)',
          ...
          {aiSummary && (
            <p style={{ fontSize: 14, color: '#ccc', lineHeight: 1.7, marginBottom: marketInsights ? 20 : 0 }}>
              {aiSummary}
            </p>
          )}
          ...
          {marketInsights && (
            <>
              ...
              {marketInsights.topCities && marketInsights.topCities.length > 0 && (
      ...
      {/* Upsell for Free â€” single unified banner */}
      {ran && isFree && (
        <div style={{ background: T.ink, borderRadius: 14, padding: '22px 26px', marginBottom: 20 }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              ...
              {experience && (
                <span style={{ fontSize: 11, fontWeight: 600, color: T.ink60,
                  background: T.paper, border: `1px solid ${T.ink10}`,
            ...
            <button onClick={handleShare} disabled={sharing}
              style={{ display: 'flex', alignItems: 'center', gap: 6, background: shareUrl ? 'rgba(40,200,64,.08)' : T.ink,
              ...
              {shareUrl ? (
                <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"/></svg>Link copiado</>
              ) : sharing ? 'Generando...' : (
                <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>Compartir</>
              )}
            </button>
          </div>
            ...
            {jobs.map((job, i) => (
              <div key={i}
                style={{ background: T.white, borderRadius: 14, padding: '18px 22px',
                  ...
                  animation: `fadeIn .35s ${i * 0.06}s ease both`, opacity: 0 }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = T.ink; e.currentTarget.style.boxShadow = '0 4px 16px rgba(0,0,0,.06)' }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
                  <div style={{ flexGrow: 1, minWidth: 0 }}>
                    ...
                    {job.reason && (
                      <p style={{ fontSize: 12, color: T.ink40, marginTop: 8, fontStyle: 'italic', lineHeight: 1.5 }}>
                        {job.reason}
                      </p>
                    )}
                    {job.tip && (
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 7, marginTop: 10,
                        ...
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                          stroke={job.relevance ? '#7B68EE' : T.green}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                    <button onClick={() => toggleFavorite(job)} title={favorites.has(job.url) ? 'Quitar de guardados' : 'Guardar oferta'}
                      style={{ width: 36, height: 36, borderRadius: 8, border: `1.5px solid ${T.ink10}`,
                        background: favorites.has(job.url) ? '#FFF8E1' : T.white,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        cursor: 'pointer', transition: 'all .15s', flexShrink: 0 }}>
                      <svg width="14" height="14" viewBox="0 0 24 24"
                        fill={favorites.has(job.url) ? '#E8A020' : 'none'}
                        stroke={favorites.has(job.url) ? '#E8A020' : T.ink40}
                        strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {!ran && !loading && !limitError && (
        <div style={{ background: T.white, borderRadius: 16, padding: '48px 32px',
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (data) { setProfile(data); setNameInput(data.full_name || '') }
      setLoading(false)
    }
    load()
  }, [])
  ...
  const NavBtn = ({ id, icon, label }: { id: View; icon: React.ReactNode; label: string }) => (
    <button onClick={() => setView(id)}
      style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '9px 10px',
        ...
        color: view === id ? T.ink : T.ink60, marginBottom: 2, transition: 'all .15s', textAlign: 'left' }}
      onMouseEnter={e => { if (view !== id) e.currentTarget.style.background = T.paper }}
      ...
      <div style={{ display: 'flex', minHeight: '100vh' }}>
        ...
        <aside style={{ width: 240, flexShrink: 0, background: T.white, borderRight: `1px solid ${T.ink10}`,
          ...
          <nav style={{ padding: '14px 12px', flexGrow: 1 }}>
            ...
            {modes.map((m, i) => (
              <button key={i}
                onClick={() => { if (m.sleep) { setSleepOpen(true); return }; if (m.available && m.id) setView(m.id) }}
                style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '9px 10px',
                ...
                onMouseEnter={e => { if (m.available && view !== m.id) e.currentTarget.style.background = T.paper }}
                onMouseLeave={e => { if (m.available && view !== m.id) e.currentTarget.style.background = 'transparent' }}>
                ...
                {!m.available && (
                  <span style={{ fontSize: 9, fontWeight: 700, color: T.ink20, background: T.paper,
          ...
          {view === 'home' && (
            <div className="fade" style={{ padding: 32 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 32 }}>
                {stats.map((s, i) => (
                  <div key={i} style={{ background: T.white, borderRadius: 14, padding: '22px 24px',
              ...
              <div style={{ background: T.ink, borderRadius: 16, padding: '24px 28px', marginBottom: 32,
                ...
                <button onClick={() => setView('opportunity')}
                  style={{ display: 'flex', alignItems: 'center', gap: 8, background: T.white, color: T.ink,
                    padding: '11px 22px', borderRadius: 9, fontSize: 14, fontWeight: 600, border: 'none',
                    fontFamily: T.sans, cursor: 'pointer', transition: 'opacity .15s' }}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 14 }}>
                {modes.map((m, i) => (
                  <div key={i}
                    onClick={() => { if (m.sleep) { setSleepOpen(true); return }; if (m.available && m.id) setView(m.id) }}
                    style={{ background: T.white, borderRadius: 16, padding: 24, border: `1px solid ${T.ink10}`,
                      cursor: m.available ? 'pointer' : 'default', transition: 'all .18s ease' }}
                    onMouseEnter={e => { if (m.available) { e.currentTarget.style.borderColor = T.ink; e.currentTarget.style.boxShadow = '0 4px 16px rgba(0,0,0,.08)' }}}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = T.ink10; e.currentTarget.style.boxShadow = 'none' }}>
                    ...
                    {m.available && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 14, fontSize: 13, fontWeight: 600, color: T.ink }}>
          ...
          {view === 'social' && profile && (
            <div className="fade">
              <SocialView
                userId={profile.id}
                supabase={supabase}
                planTier={!profile.plan || profile.plan === 'free' ? 'free' : 'pro'}
              />
            </div>
          )}

          {view === 'missions' && profile && (
            <div className="fade">
                ...
                <div style={{ background: T.white, borderRadius: 16, padding: 28, border: `1px solid ${T.ink10}` }}>
                  ...
                  <div style={{ marginBottom: 18 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      <input value={nameInput} onChange={e => setNameInput(e.target.value)} placeholder="Tu nombre"
                        style={{ flex: 1, padding: '11px 14px', borderRadius: 9, border: `1.5px solid ${T.ink10}`,
                          fontSize: 14, fontFamily: T.sans, color: T.ink, outline: 'none',
                          background: T.paper, transition: 'border-color .15s' }}
                        onFocus={e => (e.target.style.borderColor = T.ink)}
                        onBlur={e  => (e.target.style.borderColor = T.ink10)} />
