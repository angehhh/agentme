...
import { ArrowLeft, ArrowRight, Calendar, Check, CirclePlay, Clapperboard, Clock, Clipboard, Download, ExternalLink, LayoutGrid, Info, Layers, List, Music, Paperclip, Smartphone, Sparkles, Video, Zap, } from 'lucide-react';
...
function IconTile({ icon: Icon, box = 44, size = 22, background = ICON_TILE_BG, iconColor = T.ink, }: {
    icon: LucideIcon;
    box?: number;
    size?: number;
    background?: string;
...
function StrokeIcon({ icon: Icon, size = 18, color = T.ink }: {
    icon: LucideIcon;
    size?: number;
    color?: string;
}) {
...
function assignMp4sToPlanSlots(planClips: Array<Record<string, unknown>>, assets: YtCloudRenderProject[]): (YtCloudRenderProject | undefined)[] {
...
function OpusProgressOverlay({ visible, pct, variant = 'render', }: {
    visible: boolean;
    pct: number;
    ...
    const outerBg = variant === 'analysis'
        ? 'rgba(15, 23, 42, 0.78)'
        : 'rgba(6, 32, 22, 0.72)';
    const innerBg = variant === 'analysis'
        ? 'rgba(15, 35, 30, 0.96)'
        : 'rgba(6, 40, 28, 0.96)';
    return (<div style={{
            position: 'absolute',
...
function OpusClipProgressPill({ pct, etaLabel }: {
    pct: number;
    etaLabel: string;
}) {
...
function YtProjectVideoGridGateOverlay({ progress, onBypass, }: {
    progress: {
        required: number;
        ready: number;
    };
...
function sessionClipProgressFromDetail(detail: {
    ...
    const plan = detail.session.clips_plan as {
    ...
    if (required === 0)
        return { required: 0, ready: 0, complete: true };
...
function YoutubeClipPremiumInfoContent({ c }: {
    c: YoutubeVerticalClip;
}) {
    return (<>
      ...
      {c.publish_description.trim() ? (<div style={{
      ...
      {(c.suggested_hashtags.length > 0 || c.best_platforms.length > 0) && (<div style={{ marginBottom: 14 }}>
          {c.best_platforms.length > 0 && (<div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 10, alignItems: 'center' }}>
              <span style={{ fontSize: 11, fontWeight: 800, color: T.ink40, letterSpacing: '.08em' }}>PLATAFORMAS</span>
              {c.best_platforms.map((p, j) => (<span key={j} style={{
          ...
          {c.suggested_hashtags.length > 0 && (<div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
              <span style={{ fontSize: 11, fontWeight: 800, color: T.ink40, letterSpacing: '.08em', marginRight: 4 }}>HASHTAGS</span>
              {c.suggested_hashtags.map((h, j) => (<span key={j} style={{
      ...
      {c.thumbnail_cover_idea.trim() ? (<p style={{ fontSize: 13, color: T.ink60, margin: '0 0 10px', lineHeight: 1.55 }}>
          <strong style={{ color: T.ink }}>Miniatura / primer frame:</strong> {c.thumbnail_cover_idea}
        </p>) : null}

      {c.dynamic_caption_style.trim() ? (<p style={{ fontSize: 13, color: T.ink60, margin: '0 0 10px', lineHeight: 1.55 }}>
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        <ul style={{ margin: '8px 0 0', paddingLeft: 18, fontSize: 13, color: T.ink60, lineHeight: 1.5 }}>
          {c.on_screen_text_suggestions.map((t, j) => (<li key={j}>{t}</li>))}
        </ul>
      </div>
      {c.edit_checklist.length > 0 && (<div style={{ marginBottom: 14 }}>
          ...
          <ol style={{ margin: '8px 0 0', paddingLeft: 20, fontSize: 13, color: T.ink60, lineHeight: 1.55 }}>
            {c.edit_checklist.map((step, j) => (<li key={j} style={{ marginBottom: 4 }}>{step}</li>))}
          </ol>
        </div>)}
...
function YoutubeClipAutoPreviewObserver({ clipIndex, packResetKey, onEnterView, }: {
    clipIndex: number;
    packResetKey: string;
...
export default function SocialView({ userId, supabase, planTier, }: {
    ...
    const loadYtRenderSessions = useCallback(async () => {
        ...
        try {
            const res = await fetch(`/api/social/youtube-render/sessions?userId=${encodeURIComponent(userId)}`);
            const data = (await res.json()) as {
                sessions?: YtCloudSession[];
            };
            if (!aliveRef.current)
                return;
            if (Array.isArray(data.sessions))
                setYtSessions(data.sessions);
        }
        catch { }
        finally {
            if (aliveRef.current) {
    ...
    useEffect(() => {
        if (section !== 'ytclips')
            return;
        if (!ytSessionsFetchedOnce || ytSessionsLoading)
            return;
        if (ytSessions.length > 0)
            return;
        if (ytOpenSessionId)
            return;
        if (ytClipsSourceRef.current !== 'storage')
            return;
        if (!ytClips.length && !ytVideoId.trim())
            return;
    ...
    const hydrateYtCloudClipPreviews = useCallback(async () => {
        ...
        try {
            ...
            const dRes = await fetch(`/api/social/youtube-render/sessions/${encodeURIComponent(match.id)}?userId=${encodeURIComponent(userId)}`);
            ...
            if (!aliveRef.current)
                return;
            if (!detail.success || !Array.isArray(detail.assets))
                return;
            const next: Record<number, string> = {};
            for (const a of detail.assets) {
                if (a.kind !== 'mp4' || !a.download_url)
                    continue;
    ...
    const loadYtSessionDetail = useCallback(async (sessionId: string) => {
        ...
        try {
            const res = await fetch(`/api/social/youtube-render/sessions/${encodeURIComponent(sessionId)}?userId=${encodeURIComponent(userId)}`);
            const data = (await res.json()) as {
                success?: boolean;
                session?: YtCloudSession;
                assets?: YtCloudRenderProject[];
            ...
            if (aliveRef.current)
                setYtSessionDetailLoading(false);
        }
    }, [userId]);
    useEffect(() => {
        if (section === 'ytclips')
    ...
    useEffect(() => {
        if (section !== 'ytclips' || !ytVideoId || !ytClips.length)
            return;
    ...
    const savePreferences = async () => {
        ...
        if (error) {
            setSettingsErr(error.message.includes('relation') || error.code === '42P01'
    ...
    const copyHookLabAll = async () => {
        ...
        if (hookLab.sound_mood?.trim()) {
            lines.push('', '## Audio sugerido (solo guía de estilo, sin archivo de audio)', hookLab.sound_mood.trim());
        }
    ...
    function clipsToText(videoId: string, title: string | null, clips: YoutubeVerticalClip[]): string {
        ...
        clips.forEach((c, i) => {
            lines.push(`## Clip ${i + 1} · ${formatMmSs(c.start_sec)} – ${formatMmSs(c.end_sec)}`, `**${c.title}**`, `Enlace: ${youtubeWatchUrl(videoId, c.start_sec)}`, '', `Hook overlay: ${c.hook_overlay}`, `Por qué frena el scroll: ${c.why_stops_scroll}`, `Encuadre 9:16: ${c.nine_sixteen_framing}`, `Zonas seguras captions: ${c.safe_zones_caption}`, `Textos en pantalla: ${c.on_screen_text_suggestions.join(' | ')}`, `Sonido: ${c.sound_hook}`, `CTA final: ${c.cta_end}`, `Potencial (1-10): ${c.estimated_virality_1_10}`, '', `Descripción para publicar:`, c.publish_description || '—', '', `Hashtags: ${c.suggested_hashtags.length ? c.suggested_hashtags.join(' ') : '—'}`, `Plataformas recomendadas: ${c.best_platforms.length ? c.best_platforms.join(', ') : '—'}`, `Idea miniatura/cover: ${c.thumbnail_cover_idea || '—'}`, `Estilo captions dinámicos: ${c.dynamic_caption_style || '—'}`, '', `Checklist edición:`, ...(c.edit_checklist.length ? c.edit_checklist.map((x, j) => `${j + 1}. ${x}`) : ['—']), '');
        });
        return lines.join('\n');
    }
    const runYoutubeClips = async () => {
        ...
        try {
            ...
            if (clips?.length && vid) {
                ...
                try {
                    sessionStorage.setItem(storageYt, JSON.stringify({
                        videoId: vid,
    ...
    const generateYoutubeRenderClipPreview = async (clipIndex: number, opts?: {
        alsoDownload?: boolean;
    }) => {
        if (!ytUrl.trim() || !ytVideoId || !ytClips.length)
            return;
        ...
        try {
            ...
            if (ct.includes('application/json')) {
                ...
                if (!res.ok || !data.success) {
                    setYtRenderErr(typeof data.message === 'string'
                        ? data.message
                        : `No se pudo generar el vídeo del clip ${clipIndex + 1}.`);
                    return;
                }
                if (data.project?.download_url) {
                    const fname = typeof data.project.filename === 'string' && data.project.filename.trim()
                    ...
                    if (typeof data.session?.id === 'string')
                        setYtRenderSessionId(data.session.id);
                    void loadYtRenderSessions();
                    void hydrateYtCloudClipPreviews();
                    if (typeof data.session?.id === 'string' &&
    ...
    const downloadYoutubeClipFromPreview = (clipIndex: number) => {
        ...
        const fname = ytAutomatedFilenameByClip[clipIndex] ?? `clip-${clipIndex + 1}-9-16-${ytVideoId}.mp4`;
        ...
        if (cloudUrl) {
            void (async () => {
                try {
                    ...
                    setYtRenderErr('No se pudo descargar el MP4 (red o CORS). Prueba otro navegador o vuelve a generar el clip.');
                }
            })();
        }
    };
    ...
    useEffect(() => {
        if (section !== 'ytclips' || !ytVideoId || !ytClips.length)
            return;
        if (ytAutoInitialRequestedRef.current === ytVideoId)
            return;
    ...
    useEffect(() => {
        if (section !== 'ytclips' || !ytClips.length)
            return;
    ...
    const ytProjectClipsGateActive = Boolean(ytOpenSessionId &&
        ytSessionDetail &&
    ...
    useEffect(() => {
        if (!ytOpenSessionId || !ytSessionDetail || ytSessionDetailLoading)
            return;
        if (ytBypassProjectClipsGate)
            return;
        const prog = sessionClipProgressFromDetail(ytSessionDetail);
        if (prog.complete || prog.required === 0)
            return;
        const id = setInterval(() => {
    ...
    useEffect(() => {
        if (section !== 'ytclips')
            return;
        if (ytRenderingClipIndex !== null)
            return;
        if (!ytVideoId || !ytClips.length)
            return;
        if (ytPreviewDrainLockRef.current)
            return;
        const pending = ytAutoPreviewPendingRef.current;
        for (const x of [...pending]) {
            if (ytAutomatedPreviewByClip[x] || ytCloudMp4UrlByClip[x])
                pending.delete(x);
        }
    ...
    return (<div style={{ padding: 32, maxWidth: 860 }}>
      ...
      {section === 'hub' && (<>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
            ...
            <button type="button" onClick={() => setSection('hooklab')} style={{
                textAlign: 'left', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 14,
                background: T.white, borderRadius: 16, padding: '22px 24px', border: `1px solid ${T.ink10}`,
                cursor: 'pointer', fontFamily: T.sans, transition: 'border-color .15s, box-shadow .15s',
            }} onMouseEnter={e => {
            ...
            <button type="button" onClick={() => setSection('ytclips')} style={{
                textAlign: 'left', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 14,
                background: T.white, borderRadius: 16, padding: '22px 24px', border: `1px solid ${T.ink10}`,
                cursor: 'pointer', fontFamily: T.sans, transition: 'border-color .15s, box-shadow .15s',
            }} onMouseEnter={e => {
      ...
      <div style={{ background: T.white, borderRadius: 16, padding: '24px 28px', border: `1px solid ${T.ink10}`, marginBottom: 24 }}>
        ...
        {settingsLoading ? (<p style={{ color: T.ink40 }}>Cargando…</p>) : (<>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
              <button type="button" onClick={savePreferences} disabled={savingPrefs} style={{
      ...
      {calendar && (<section style={{ marginBottom: 32 }}>
          <div style={{
          ...
          <div style={{
            ...
            {!isPro && (<p style={{ fontSize: 13, color: T.coral, margin: 0, fontWeight: 600, opacity: 0.95 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 20, alignItems: 'center' }}>
              <button type="button" onClick={copyWeek} style={{ ...btnGhost, padding: '10px 18px', fontSize: 13 }}>
                {copied || copiedFlash === 'week' ? (<span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                    <StrokeIcon icon={Ico.check} size={18} color={T.green}/> Copiado al portapapeles
                  </span>) : (<span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            {calendar.posts.map((p, idx) => (<article key={`${p.day}-${idx}`} style={{
                        background: T.white,
                        borderRadius: 18,
                        overflow: 'hidden',
                        border: `1px solid ${T.ink10}`,
                ...
                <div style={{ padding: '20px 22px 22px' }}>
                  ...
                  <div style={{ marginBottom: 18 }}>
                    ...
                    <ol style={{ margin: 0, padding: 0, listStyle: 'none' }}>
                      {p.outline.map((line, i) => (<li key={i} style={{
                  ...
                  {p.hashtags && (<div style={{ marginBottom: 16 }}>
                      <SectionLabel color={T.violet}>Hashtags</SectionLabel>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                        {splitHashtagTokens(p.hashtags).map((tag, ti) => (<span key={ti} style={{
      ...
      {hookLab && (<section style={{ marginBottom: 32 }}>
          <div style={{
                    display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16,
                    paddingBottom: 12, borderBottom: `1px solid ${T.ink10}`,
                }}>
          ...
          <div style={{
            ...
            <button type="button" onClick={copyHookLabAll} style={{ ...btnGhost, padding: '10px 18px', fontSize: 13 }}>
              {hookCopied || copiedFlash === 'hooklab' ? (<span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ marginBottom: 28 }}>
            ...
            <div style={{
              ...
              {hookLab.hooks.map((h, i) => (<div key={i} style={{
          ...
          {hookLab.on_screen_texts && hookLab.on_screen_texts.length > 0 && (<div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {hookLab.on_screen_texts.map((t, i) => (<div key={i} style={{
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {hookLab.angles.map((a, i) => (<div key={i} style={{
      ...
      <div style={{
        ...
        {ytSessionsLoading && ytSessions.length === 0 ? (<p style={{ margin: 0, fontSize: 13, color: '#888' }}>Cargando proyectos…</p>) : null}

        {!ytSessionsLoading && ytSessions.length === 0 ? (<p style={{ margin: 0, fontSize: 13, color: '#888', lineHeight: 1.55 }}>
        ...
        {!ytSessionsLoading && ytSessions.length > 0 ? (<div style={{
                    display: 'grid',
                    gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))',
                    gap: 16,
                }}>
            {ytSessions.map(s => {
                    const showProgress = (ytRenderingClipIndex !== null || ytRenderZipLoading) &&
                        s.youtube_video_id === ytVideoId;
                    const planForDur = s.clips_plan as {
                        clips?: {
                            end_sec?: unknown;
                        }[];
                    } | undefined;
                    const durClips = planForDur?.clips;
                    const approxSourceMin = Array.isArray(durClips) && durClips.length > 0
                        ? Math.max(1, Math.ceil(Math.max(...durClips.map(c => Number(c?.end_sec) || 0)) / 60))
                        : null;
                  ...
                  <div style={{ position: 'relative', aspectRatio: '16/9', background: '#000' }}>
                    ...
                    <div style={{
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'flex-end' }}>
                        {approxSourceMin != null ? (<span style={{
                                background: 'rgba(0,0,0,.78)',
                                color: '#e8e8e8',
                                fontSize: 10,
                                fontWeight: 800,
        ...
        {ytOpenSessionId ? (<div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 14 }}>
              <div>
                ...
                {ytSessionDetail ? (<p style={{ margin: '6px 0 0', fontSize: 12, color: '#999', lineHeight: 1.5 }}>
            ...
            {ytSessionDetailLoading ? (<p style={{ color: '#888', fontSize: 13 }}>Cargando información…</p>) : null}

            <div style={{ position: 'relative' }}>
              {ytSessionDetail && !ytSessionDetailLoading ? (<div style={{
                        display: 'flex',
                        flexDirection: 'column',
                        gap: 18,
                    }}>
                {!ytProjectClipsGateActive
                        ? ytSessionDetail.assets.filter(a => a.kind === 'zip').map(z => (<div key={z.id} style={{
                                padding: 14,
                                borderRadius: 10,
                                background: '#222',
                                border: '1px solid #333',
                    ...
                    {z.download_url ? (<a href={z.download_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, fontWeight: 700, color: '#7dd3a0' }}>
                        Descargar ZIP
                      </a>) : null}
                  </div>))
                        : null}
                ...
                {(() => {
                        const plan = ytSessionDetail.session.clips_plan as {
                            clips?: Array<Record<string, unknown>>;
                        } | null;
                        const planClips = Array.isArray(plan?.clips) ? plan.clips : [];
                        ...
                        const gateOverlay = ytProjectClipsGateActive && ytSessionClipProgress ? (<YtProjectVideoGridGateOverlay progress={ytSessionClipProgress} onBypass={() => setYtBypassProjectClipsGate(true)}/>) : null;
                        const gridWrapStyle: CSSProperties = {
                            position: 'relative',
                            minHeight: ytProjectClipsGateActive ? 280 : undefined,
                        };
                        if (planClips.length === 0) {
                            return (<div style={gridWrapStyle}>
                        {gateOverlay}
                        <p style={{
                                    margin: '4px 0 10px',
                                    fontSize: 11,
                        ...
                        <div style={{
                          ...
                          {assets.map((a, assetIdx) => {
                                ...
                                <div style={{ position: 'relative', aspectRatio: '9/16', background: '#000', flexShrink: 0 }}>
                                  {a.download_url ? (<video src={a.download_url} controls playsInline style={vidStyle}/>) : (<div style={{
                                                height: '100%',
                                                display: 'flex',
                                                alignItems: 'center',
                                                justifyContent: 'center',
                                  ...
                                  <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
                                    <div style={{
                                      ...
                                      <button type="button" onClick={e => {
                                        ...
                                        INFORMACIÓN
                                        {!isPro ? (<span style={{ fontSize: 8, fontWeight: 800, opacity: 0.9 }}>+P</span>) : null}
                                      </button>
                                    </div>
                                    {a.download_url ? (<button type="button" onClick={e => {
                                                e.stopPropagation();
                      ...
                      <div style={{
                        ...
                        {planClips.map((row, i) => {
                                ...
                                const openCloudInfo = () => {
                                    const shaped = ensureYoutubeClipShape(row as Partial<YoutubeVerticalClip>);
                                    const timing = shaped.end_sec > shaped.start_sec
                                ...
                                return (<div key={`plan-${idx}-${i}`} style={{
                                        borderRadius: 12,
                                        overflow: 'hidden',
                                        background: '#222',
                                        border: '1px solid #333',
                              ...
                              <div style={{ position: 'relative', aspectRatio: '9/16', background: '#000', flexShrink: 0 }}>
                                {mp4?.download_url ? (<video src={mp4.download_url} controls playsInline style={vidStyle}/>) : (<div style={{
                                            height: '100%',
                                            display: 'flex',
                                            alignItems: 'center',
                                            justifyContent: 'center',
                                ...
                                <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
                                  ...
                                  {mp4?.download_url ? (<button type="button" onClick={e => {
                                            e.stopPropagation();
      ...
      <div style={{ background: T.white, borderRadius: 16, padding: '24px 28px', border: `1px solid ${T.ink10}`, marginBottom: 24 }}>
        ...
        <input value={ytUrl} onChange={e => setYtUrl(e.target.value)} placeholder="YouTube: https://youtube.com/watch?v=… · youtu.be/…" style={{ ...inputStyle, marginBottom: 14 }}/>
        <label style={{ fontSize: 11, fontWeight: 700, color: T.ink40, display: 'block', marginBottom: 6 }}>
          Idioma de subtítulos (opcional, ej. es, en)
        </label>
        <input value={ytCaptionLang} onChange={e => setYtCaptionLang(e.target.value)} placeholder="Dejar vacío = idioma por defecto del vídeo" style={{ ...inputStyle, marginBottom: 14 }}/>
        <label style={{ fontSize: 11, fontWeight: 700, color: T.ink40, display: 'block', marginBottom: 6 }}>
        ...
        <button type="button" onClick={runYoutubeClips} disabled={ytLoading || !ytUrl.trim()} style={{
                padding: '10px 22px', borderRadius: 9, border: 'none',
        ...
        {ytLoading ? (() => {
              ...
              {analysisVid ? (<img src={youtubeThumbUrl(analysisVid)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}/>) : null}
      ...
      {ytClips.length > 0 && ytVideoId && (<section style={{ marginBottom: 32 }}>
          <div style={{
                    display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16,
            ...
            <button type="button" onClick={copyYtClipsAll} style={{ ...btnGhost, padding: '10px 18px', fontSize: 13 }}>
              {ytCopied || copiedFlash === 'ytclips' ? (<span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{
            ...
            <button type="button" onClick={() => { void downloadYoutubeRenderZip(); }} disabled={ytRenderZipLoading || ytRenderingClipIndex !== null || !ytUrl.trim()} style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            {ytClips.map((c, i) => (<article key={i} style={{
                        position: 'relative',
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
                  ...
                  <a href={youtubeWatchUrl(ytVideoId, c.start_sec)} target="_blank" rel="noopener noreferrer" style={{
                        display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 600,
                ...
                <div style={{
                  ...
                  <div style={{ flexShrink: 0, width: '100%', maxWidth: 240 }}>
                    <div style={{
                      ...
                      {(ytAutomatedPreviewByClip[i] ?? ytCloudMp4UrlByClip[i]) ? (<video key={ytAutomatedPreviewByClip[i] ?? ytCloudMp4UrlByClip[i] ?? `clip-${i}`} src={ytAutomatedPreviewByClip[i] ?? ytCloudMp4UrlByClip[i]} controls playsInline preload="metadata" style={{
                            position: 'absolute',
                            inset: 0,
                            width: '100%',
                            height: '100%',
                      ...
                      {ytRenderingClipIndex === i ? (<OpusProgressOverlay visible pct={ytRenderFakeProgress} variant="render"/>) : null}
                    </div>
                    <details style={{ marginTop: 10, fontSize: 11, color: T.ink40, fontFamily: T.sans }}>
                      ...
                      <div style={{
                        ...
                        <iframe title={`Referencia YouTube clip ${i + 1}`} src={youtubeEmbedClipUrl(ytVideoId, c.start_sec, c.end_sec, ytEmbedOrigin ? { origin: ytEmbedOrigin } : undefined)} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen loading="lazy" referrerPolicy="strict-origin-when-cross-origin" style={{ width: '100%', height: '100%', border: 'none', display: 'block' }}/>
                      </div>
                      <p style={{ margin: '8px 0 0', fontSize: 11, color: T.ink40, lineHeight: 1.45 }}>
                        ...
                        <a href={youtubeWatchUrl(ytVideoId, c.start_sec)} target="_blank" rel="noopener noreferrer" style={{ color: T.coral, fontWeight: 700 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 180 }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
                      <button type="button" onClick={() => downloadOrRenderYoutubeClipMp4(i)} disabled={ytRenderingClipIndex !== null ||
                        ytRenderZipLoading ||
                        !ytUrl.trim()} style={{
                        display: 'inline-flex',
                        alignItems: 'center',
                        ...
                        fontSize: 13,
                        fontFamily: T.sans,
                        cursor: ytRenderingClipIndex !== null || ytRenderZipLoading || !ytUrl.trim()
                            ? 'wait'
                            : 'pointer',
                        opacity: ytRenderingClipIndex !== null || ytRenderZipLoading || !ytUrl.trim()
                            ? 0.65
                            : 1,
                    }}>
                      ...
                      {(ytAutomatedPreviewByClip[i] ?? ytCloudMp4UrlByClip[i]) ? (<button type="button" onClick={() => { void generateYoutubeRenderClipPreview(i, { alsoDownload: true }); }} disabled={ytRenderingClipIndex !== null ||
                            ytRenderZipLoading ||
                            !ytUrl.trim()} style={{
                            display: 'inline-flex',
                            alignItems: 'center',
                ...
                <div style={{
                  ...
                  <button type="button" onClick={() => {
                    ...
                    INFORMACIÓN
                    {!isPro ? (<span style={{ fontSize: 10, fontWeight: 700, opacity: 0.85 }}>+ Pro</span>) : null}
                  </button>
                  {!isPro ? (<span style={{ fontSize: 12, color: T.ink40, lineHeight: 1.45 }}>
      ...
      {ytClipInfoModalIndex !== null && ytClips[ytClipInfoModalIndex] ? (<div role="presentation" style={{
                    position: 'fixed',
                    inset: 0,
                    zIndex: 10040,
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="yt-clip-info-title" onClick={e => e.stopPropagation()} style={{
                    width: '100%',
                    maxWidth: 560,
            ...
            <div style={{
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <h3 id="yt-clip-info-title" style={{
                    margin: '6px 0 0',
                    fontFamily: T.serif,
                    fontSize: 18,
                    fontWeight: 700,
              ...
              <button type="button" onClick={() => setYtClipInfoModalIndex(null)} style={{
                    flexShrink: 0,
                    padding: '8px 12px',
                    borderRadius: 8,
                    border: `1px solid ${T.ink10}`,
            ...
            <div style={{
              ...
              {isPro ? (<YoutubeClipPremiumInfoContent c={ytClips[ytClipInfoModalIndex]}/>) : (<div style={{
                        padding: '16px 18px',
                        borderRadius: 12,
                        background: T.paper,
      ...
      {ytCloudClipInfoModal ? (<div role="presentation" style={{
                    position: 'fixed',
                    inset: 0,
                    zIndex: 10041,
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="yt-cloud-clip-info-title" onClick={e => e.stopPropagation()} style={{
                    width: '100%',
                    maxWidth: ytCloudClipInfoModal.premiumClip ||
                        (isPro && ytCloudClipInfoModal.fallbackDesc)
                        ? 560
                        : 520,
                    maxHeight: ytCloudClipInfoModal.premiumClip ||
                        (isPro && ytCloudClipInfoModal.fallbackDesc)
                        ? 'min(90vh, 820px)'
                        : 'min(88vh, 680px)',
                    overflow: 'hidden',
            ...
            <div style={{
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <h3 id="yt-cloud-clip-info-title" style={{
                    margin: '6px 0 0',
                    fontFamily: T.serif,
                    fontSize: 18,
                    fontWeight: 700,
              ...
              <button type="button" onClick={() => setYtCloudClipInfoModal(null)} style={{
                    flexShrink: 0,
                    padding: '8px 12px',
                    borderRadius: 8,
                    border: '1px solid #444',
            ...
            <div style={{
              ...
              {ytCloudClipInfoModal.premiumClip ? (<div style={{
                        marginBottom: 16,
              ...
              {isPro && ytCloudClipInfoModal.fallbackDesc ? (<p style={{
                        margin: '0 0 16px',
                        fontSize: 13,
                        color: '#9ca3af',
                        lineHeight: 1.55,
              ...
              {ytCloudClipInfoModal.downloadUrl ? (<button type="button" onClick={() => {
                        downloadCloudSessionMp4(ytCloudClipInfoModal.downloadUrl!, ytCloudClipInfoModal.filenameHint);
                    }} style={{
                        marginTop: 0,
                        display: 'inline-flex',
                        alignItems: 'center',
                  ...
                  <Download size={18} strokeWidth={1.75} aria-hidden/>
                  Descargar MP4
                </button>) : (<p style={{ margin: '16px 0 0', fontSize: 12, color: '#6b7280', lineHeight: 1.5 }}>
