...
export async function POST(req: NextRequest) {
  try {
    ...
    if (isFree) {
      ...
      if (cErr) console.error('[social/youtube-clips] count', cErr)
      const used = count ?? 0
    ...
    if (timed.length > MAX_TIMED_CHARS) {
      timed =
        ...
        '\n\nâ€¦ [Transcripción truncada: el vídeo es muy largo; los clips se basan en la parte inicial.]'
    }
    ...
    if (mErr) console.error('[social/youtube-clips] mission insert', mErr)
