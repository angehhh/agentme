...
function isYoutubeRenderDisabled(): boolean {
    if (process.env.DISABLE_YOUTUBE_RENDER === '1')
        return true;
    if (process.env.VERCEL === '1' && process.env.ENABLE_YOUTUBE_RENDER !== '1')
        return true;
    return false;
}
...
async function assertRenderLimit(userId: string, tier: 'free' | 'pro'): Promise<NextResponse | null> {
    ...
    if (cErr)
        console.error('[social/youtube-render] count', cErr);
    const used = count ?? 0;
...
async function jsonStoredProject(row: YoutubeRenderProjectRow) {
    ...
    if (sErr)
        console.error('[social/youtube-render] signed url', sErr);
    return {
        ...row,
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!videoId) {
            return NextResponse.json({ error: 'URL inválida', message: 'Pega un enlace válido de YouTube.' }, { status: 400 });
        }
        if (output === 'mp4') {
            if (clips.length !== 1) {
                return NextResponse.json({
        ...
        if (store) {
            ...
            if ('error' in sr) {
                return NextResponse.json({ error: 'session_error', message: sr.error }, { status: 503 });
            }
        ...
        try {
            if (output === 'mp4' && singleClip) {
                ...
                if (store && sessionForStore) {
                    ...
                    if ('error' in saved) {
                        return NextResponse.json({ error: 'storage_fallido', message: saved.error }, { status: 503 });
                    }
