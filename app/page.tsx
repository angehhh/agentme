...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   ICON LIBRARY  â€” pure SVG, zero dependencies
   â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
const IC = {
  logo: (
...
const T = {
  ...
  paper:     '#F5F4F1',
  white:     '#FFFFFF',
  green:     '#28C840',     // single status accent â€” never decorative
  yellow:    '#E8E350',     // "most popular" badge only
...
export default function AgentMeLanding() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      if (i >= goal.length) { setCaretOn(false); clearInterval(iv) }
    }, 46)
  ...
  const Section = ({ id, bg, children, pt = 130, pb = 130 }:
    { id?: string; bg?: string; children: React.ReactNode; pt?: number; pb?: number }) => (
  ...
  const Btn = ({ label, onClick, variant = 'solid', size = 'md', icon }:
    { label: string; onClick: () => void; variant?: 'solid' | 'outline' | 'ghost'; size?: 'md' | 'lg'; icon?: React.ReactNode }) => {
    ...
    return (
      <button style={styles[variant]} onClick={onClick}
        onMouseEnter={e => {
          if (variant === 'solid')   { e.currentTarget.style.opacity = '.85' }
          if (variant === 'outline') { e.currentTarget.style.borderColor = T.ink }
          if (variant === 'ghost')   { e.currentTarget.style.color = T.ink }
        }}
        onMouseLeave={e => {
          if (variant === 'solid')   { e.currentTarget.style.opacity = '1' }
          if (variant === 'outline') { e.currentTarget.style.borderColor = T.ink10 }
          if (variant === 'ghost')   { e.currentTarget.style.color = T.ink50 }
        }}>
        {label}{icon}
      </button>
    )
      ...
      <div>
            ...
            <div style={{ display: 'flex', gap: 34 }}>
              {[['CÃ³mo funciona','como-funciona'],['Modos','modos'],['Precios','precios']].map(([l,id]) => (
                <button key={id} onClick={() => scroll(id)}
                  ...
                  onMouseLeave={e => (e.currentTarget.style.color = T.ink50)}>{l}</button>
              ))}
            </div>
            ...
            <div style={{ borderRadius: 18, overflow: 'hidden',
              ...
              <div style={{ background: '#161616', borderBottom: '1px solid #242424',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 9,
                  ...
                  <Dot color="#28C840" />
                  agentme.app â€” sleep_mode â€” activo
                </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
                    {[
                      ...
                      { icon: IC.activity, v: '100%', l: 'Ã‰xito'    },
                    ].map(s => (
                      <div key={s.l} style={{ background: '#161616', borderRadius: 10,
          ...
          <div style={{ maxWidth: 960, margin: '0 auto', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap',
              ...
              {['LinkedIn','Supabase','Resend','Claude AI'].map(p => (
                <span key={p}
                  style={{ fontSize: 15, fontWeight: 500, color: T.ink30,
                    transition: 'color .15s', cursor: 'default' }}
                  onMouseEnter={e => (e.currentTarget.style.color = T.ink)}
                  onMouseLeave={e => (e.currentTarget.style.color = T.ink30)}>{p}</span>
              ))}
            </div>
          </div>
        </section>
        ...
        <Section id="modos" bg={T.white}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between',
            ...
            <Btn label="Ver todos" onClick={() => router.push('/auth')}
              variant="solid" icon={IC.arrowRight} />
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
            ...
            <div style={{ background: T.ink, borderRadius: 20, padding: 52,
              ...
              <div style={{ paddingTop: 36 }}>
                <Btn label="Probar gratis" onClick={() => router.push('/auth')}
                  variant="outline"
                  icon={IC.arrowRight} />
              </div>
            </div>
        ...
        <Section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
            {testimonials.map(t => (
              <div key={t.n} style={{ background: T.white, borderRadius: 16,
        ...
        <Section id="precios" bg={T.white}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)',
            ...
            {plans.map(plan => (
              <div key={plan.name} style={{ borderRadius: 16, padding: 34,
          ...
          <div style={{ maxWidth: 1120, margin: '0 auto' }}>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 22 }}>
                  {[IC.twitter, IC.linkedin, IC.globe].map((ico, i) => (
                    <div key={i} style={{ width: 34, height: 34, borderRadius: 8,
                      ...
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      color: T.ink50, cursor: 'pointer', transition: 'all .15s' }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = T.ink; (e.currentTarget as HTMLElement).style.color = T.white }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = T.paper; (e.currentTarget as HTMLElement).style.color = T.ink50 }}>
                      {ico}
                    </div>
                  ))}
                </div>
                ...
                {[
                  ...
                  <div key={col.title}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {col.links.map(l => (
                        <a key={l} href="#"
                          style={{ fontSize: 14, color: T.ink30, transition: 'color .15s' }}
                          onMouseEnter={e => (e.currentTarget.style.color = T.ink)}
                          onMouseLeave={e => (e.currentTarget.style.color = T.ink30)}>{l}</a>
                      ))}
                    </div>
                  </div>
                ))}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between',
              ...
              <p style={{ fontSize: 13, color: T.ink30 }}>
                Â© 2026 AGENTME. Todos los derechos reservados.
              </p>
