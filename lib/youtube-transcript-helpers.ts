...
/**
 * YouTube puede devolver tiempos en ms (srv3) o en segundos (formato clásico).
 * Heurística: duraciones grandes en enteros → milisegundos.
 */
export function cueToSeconds(item: TranscriptResponse): { startSec: number; endSec: number } {
  ...
  /** Enteros: InnerTube suele usar ms (p. ej. duración 3200 = 3,2 s). Enteros &lt; 200 suelen ser segundos. */
  if (duration >= 200) {
...
/** Línea de tiempo legible para el modelo (segundos con 1 decimal) */
export function transcriptToTimedLines(segments: TranscriptResponse[], maxLines = 900): string {
  ...
  for (const seg of segments) {
    ...
    if (!t) continue
    lines.push(`[${startSec.toFixed(1)}s–${endSec.toFixed(1)}s] ${t}`)
    if (lines.length >= maxLines) {
      lines.push('… (transcripción truncada por longitud)')
      break
    }
  }
  return lines.join('\n')
