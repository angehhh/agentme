...
export function resolveYtDlpExecutable(): string {
    ...
    if (fromEnv && existsSync(fromEnv))
        return fromEnv;
    const inProject = path.join(process.cwd(), 'node_modules', 'youtube-dl-exec', 'bin', process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp');
    ...
    if (fromPath)
        return fromPath;
    throw new Error('No se encontró yt-dlp. Opciones: (1) En la carpeta del proyecto ejecuta `npm install`. ' +
...
export function resolveFfmpegPath(): string {
    ...
    if (fromEnv && existsSync(fromEnv))
        return fromEnv;
    const inProject = path.join(process.cwd(), 'node_modules', 'ffmpeg-static', process.platform === 'win32' ? 'ffmpeg.exe' : 'ffmpeg');
    if (existsSync(inProject))
        return inProject;
    if (ffmpegStatic && typeof ffmpegStatic === 'string' && existsSync(ffmpegStatic)) {
        return ffmpegStatic;
    }
    const fromPath = findOnPath('ffmpeg') || (process.platform === 'win32' ? findOnPath('ffmpeg.exe') : null);
    if (fromPath)
        return fromPath;
    throw new Error('No se encontró FFmpeg. Define FFMPEG_BIN, o ejecuta `npm install` en la raíz del proyecto, ' +
