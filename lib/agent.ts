...
function getRelevantKeywords(query: string): string[] {
  ...
  for (const [sector, keywords] of Object.entries(SECTOR_KEYWORDS)) {
    if (q.includes(sector) || keywords.some(k => q.includes(k))) return keywords
  }
...
function isRelevant(title: string, query: string): boolean {
  ...
  const q = query.toLowerCase()
  if (q.split(/\s+/).filter(w => w.length > 2).some(w => t.includes(w))) return true
  if (getRelevantKeywords(query).some(k => t.includes(k))) return true
  return false
}
...
async function scrapeUrl(page: Page, url: string, limit: number): Promise<ScrapeResult> {
  ...
  const data = await page.evaluate((maxJobs: number) => {
    ...
    cards.forEach((card, i) => {
      ...
      if (title && url) results.push({ title, company, location, url, posted })
    })
    ...
    if (!totalLinkedIn) {
      ...
      for (const h of headings) {
        ...
        if (match) { totalLinkedIn = parseInt(match[1].replace(/[,.]/g, ''), 10); break }
      }
    }

    const jobs = results.filter((job, i, self) => i === self.findIndex(j => j.url === job.url))
    return { jobs, totalLinkedIn }
  }, limit) as { jobs: JobResult[]; totalLinkedIn: number }
