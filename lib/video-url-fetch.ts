...
export async function fetchVideoFromUrl(videoUrl: string): Promise<VideoFetchResult> {
    ...
    if (hostBlocked(url.hostname)) {
        return {
            ok: false,
            message: 'Esa plataforma no permite descarga directa desde aquí. Sube el archivo MP4 o usa un enlace directo a un .mp4 (CDN, almacenamiento propio, etc.).',
        };
    }
    ...
    try {
        ...
        const looksVideo = ct.startsWith('video/') ||
            path.endsWith('.mp4') ||
            path.endsWith('.webm') ||
            path.endsWith('.mov') ||
            path.endsWith('.m4v');
