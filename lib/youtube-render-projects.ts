...
export function defaultExpiresAtIso(): string {
    return new Date(Date.now() + YOUTUBE_RENDER_TTL_DAYS * 24 * 60 * 60 * 1000).toISOString();
}
...
export function formatExpiresLabelEs(expiresAtIso: string, nowMs: number = Date.now()): string {
    ...
    if (h >= 48)
        return `${YOUTUBE_RENDER_TTL_DAYS} días antes de caducar`;
    if (h >= 36)
        return '1 día antes de caducar';
    if (h >= 24)
        return 'Caduca en 1 día';
    if (h >= 2)
        return `Caduca en ${Math.ceil(h)} h`;
    if (h >= 1)
        return 'Caduca en ~1 h';
...
export async function getOrCreateYoutubeRenderSession(params: {
    ...
    session: YoutubeRenderSessionRow;
} | {
    error: string;
}> {
    ...
    const patchFromPlan = async (sessionId: string) => {
        ...
        if (params.videoTitle?.trim())
            patch.video_title = params.videoTitle.trim();
        if (params.youtubeUrl?.trim())
            patch.youtube_url = params.youtubeUrl.trim();
    ...
    if (insErr || !inserted) {
        ...
        const code = String((insErr as {
            code?: string;
        })?.code ?? '');
        const msg = String((insErr as {
            message?: string;
        })?.message ?? '');
        const missingSessionsTable = code === 'PGRST205' ||
...
async function signProjectRow(supabase: SupabaseClient, row: YoutubeRenderProjectRow): Promise<YoutubeRenderProjectPublic> {
    ...
    if (sErr)
        console.error('[youtube_render_projects] signed url', sErr);
    return {
        ...row,
...
export async function cleanupExpiredYoutubeRenderSessions(supabase: SupabaseClient): Promise<number> {
    const now = new Date().toISOString();
    ...
    for (const s of sessions || []) {
        ...
        for (const a of assets || []) {
            ...
            if (rmErr)
                console.error('[youtube_render_sessions] storage remove', rmErr);
        }
        ...
        if (!delErr)
            ...
            console.error('[youtube_render_sessions] delete', delErr);
    }
    return removed;
}
export async function cleanupOrphanYoutubeRenderProjects(supabase: SupabaseClient): Promise<number> {
    const now = new Date().toISOString();
...
export async function listYoutubeRenderSessionsForUser(supabase: SupabaseClient, userId: string): Promise<YoutubeRenderSessionCard[]> {
    ...
    for (const sid of ids)
        counts.set(sid, { total: 0, ready: 0 });
    for (const a of assets || []) {
...
export async function getYoutubeRenderSessionDetail(supabase: SupabaseClient, userId: string, sessionId: string): Promise<YoutubeRenderSessionDetail | null> {
    await runFullYoutubeRenderCleanup(supabase);
    const now = new Date().toISOString();
...
export async function listYoutubeRenderProjectsForUser(supabase: SupabaseClient, userId: string): Promise<YoutubeRenderProjectPublic[]> {
    await runFullYoutubeRenderCleanup(supabase);
    const now = new Date().toISOString();
    const { data: rows, error } = await supabase
        .from('youtube_render_projects')
...
export async function saveYoutubeRenderToStorage(params: {
    ...
    project: YoutubeRenderProjectRow;
} | {
    error: string;
}> {
