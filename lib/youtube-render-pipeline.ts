...
function runFfmpeg916(ff: string, input: string, output: string, startSec: number, durationSec: number): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        ...
        p.on('close', code => {
            if (code === 0)
                ...
                reject(new Error(`ffmpeg terminó con código ${code}: ${err.slice(-1200)}`));
        });
    });
}
async function zipFilesToBuffer(entries: {
    abs: string;
    name: string;
}[]): Promise<Buffer> {
...
export function validateRenderClips(clips: YoutubeRenderClip[]): {
    ok: true;
} | {
    ok: false;
    message: string;
...
async function downloadYoutubeToTmp(params: {
    ...
    const sourceFile = names.find(f => f.startsWith('source.') && !f.endsWith('.part') && !f.endsWith('.ytdl'));
    if (!sourceFile) {
...
export async function renderYoutubeSingleClipToMp4Buffer(params: {
    ...
    try {
        ...
        if (dur > YOUTUBE_RENDER_MAX_SOURCE_DURATION_SEC) {
            throw new Error(`El vídeo dura más de ${YOUTUBE_RENDER_MAX_SOURCE_DURATION_SEC / 3600} h; reduce la duración o elige otro.`);
        }
